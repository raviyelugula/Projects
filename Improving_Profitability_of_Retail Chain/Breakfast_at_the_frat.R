...
n =names(output)
for(i in Prod_lis){
  data1=subset(data,UPC=='1111035398')
  data_a=data1[,c(7,10,11,12)]
  data_a$FEATURE=as.factor(data_a$FEATURE)
  data_a$DISPLAY=as.factor(data_a$DISPLAY)
  data_a$TPR_ONLY=as.factor(data_a$TPR_ONLY)
...
P_TOP_LIST = c('1600027527','3800031838','7192100339',
               '1600027528','1600027564','3800031829',
               '3800039118','7192100337','1111085350',
               '88491201426','1111009477')
#for(Product in P_TOP_LIST){
Product=P_TOP_LIST[9]
print(Product)
...
Predicted_SPEND = forecast::forecast(ARIMA_Train, h=38)
Acc=accuracy(TS_data_test,Predicted_SPEND$mean)
Acc

#Holt Winter's Forecast

sales.HW= HoltWinters(TS_data_train, alpha=0.2, beta=0.9, gamma=0.5)
sales.fs.HW= forecast(sales.HW, 38)
plot(sales.fs.HW)
acc.HW=accuracy(sales.fs.HW$mean,TS_data_test)
acc.HW

#TbATS Model

sales.tbats = tbats(TS_data_train)
sales.fs.tbats = forecast(sales.tbats, h=38)
acc.tb=accuracy(sales.fs.tbats,TS_data_test)
plot(sales.fs.tbats)
acc.tb

#NNETS Model

sales.nn = nnetar(TS_data_train)
sales.fs.nn = forecast(sales.nn, h=38)
acc.nn=accuracy(sales.fs.nn,TS_data_test)
plot(sales.fs.nn)
acc.nn
...
colnames(Product_forecast) = c('2012W2','2012W3','2012W4','2012W5','2012W6','2012W7','2012W8','2012W9','2012W10','2012W11','2012W12','2012W13')
...
# Productwise Spend forecast for next 12 Weeks----
Product_forecast = data.frame(numeric())
Product_accr_AA = data.frame(numeric())
Product_accr_TB = data.frame(numeric())
Product_accr_NN = data.frame(numeric())
...
for(Product in P_TOP_LIST){
  ...
  Predicted_SPEND = forecast::forecast(ARIMA_Train, h=38)
  Acc=accuracy(TS_data_test,Predicted_SPEND$mean)
  Product_accr_AA = rbind(Product_accr_AA,Acc)
  ...
  sales.fs.tbats = forecast(sales.tbats, h=38)
  acc.tb=accuracy(sales.fs.tbats,TS_data_test)
  Product_accr_TB = rbind(Product_accr_TB,acc.tb)
  ...
  sales.fs.nn = forecast(sales.nn, h=38)
  acc.nn=accuracy(sales.fs.nn,TS_data_test)
  Product_accr_NN= rbind(Product_accr_NN,acc.nn)
  
  ARIMA_Full = auto.arima(TS_data)
  Forecast_12W = forecast::forecast(ARIMA_Full, h=12 )
  Forecast_12W$mean
  #print(Forecast_12W$mean)
  Product_forecast_AA = rbind(Product_forecast_AA,Forecast_12W$mean)
  name = paste('AA',Product,'_Forecast_SPEND_12W.png', sep = '')
  png(filename =paste0('plots/',name), width = 800, height = 600, units = 'px')
  plot(Forecast_12W$mean,type='b', xlab='TIME',ylab='SPEND',main= Product)
  dev.off()
  nn_Full = nnetar(TS_data)
  Forecast_12W_nn = forecast::forecast(nn_Full, h=12 )
  Forecast_12W_nn$mean
  #print(Forecast_12W$mean)
  Product_forecast_NN= rbind(Product_forecast_NN,Forecast_12W_nn$mean)
  name = paste('NN',Product,'_Forecast_SPEND_12W.png', sep = '')
  png(filename =paste0('plots/',name), width = 800, height = 600, units = 'px')
  plot(Forecast_12W$mean,type='b', xlab='TIME',ylab='SPEND',main= Product)
  dev.off()
}
