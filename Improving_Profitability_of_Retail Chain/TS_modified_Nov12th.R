...
prod_eff = data.frame(numeric(),numeric())
for(i in P_TOP_LIST){
  ...
  P1_PROMO_TREND = P1_Promo %>% 
    ...
    dplyr::select(WEEKEND_DATE,PROMO_SPEND) %>%
    unique()
  ...
  P2 = left_join(P1_PROMO_TREND,P1_data, by = 'WEEKEND_DATE')
  print(min(P2$PROMO_SPEND))
  PE = mean((P2$INCREMENTAL/P2$PROMO_SPEND)*100)
  print(PE)
  prod_eff = rbind(prod_eff,data.frame(i,PE))
  print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
}
colnames(prod_eff) = c('UPC','PE')
