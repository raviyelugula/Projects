...
###1.Introduction
We focused on fetching the tweets with `#Nokia` from twitter and analyse the sentiments.
...
* Pull recent 2,500 tweets containing the hash tag **#Nokia**.
...
```{r}
index <- which(dimnames(Tweets_Corpus_TDM)$Terms %in% c("android", "brand"))
...
```{r warning=FALSE}
Nokia_Associations = findAssocs(Tweets_Corpus_TDM,terms ='android',corlimit=0.2)
Nokia_Associations
...
ggplot(Nokia_Associations_df, aes(y = Nokia_Associations_df[, 1])) + 
  ...
  xlab('Correlation')+ylab('Words')+ggtitle('Nokia tweets Association with "android" ')
...
qheat(Nokia_Associations_df, values=TRUE, high="blue",
  ...
  xlab('Words')+ggtitle('Nokia tweets HeatMap with "android"')
